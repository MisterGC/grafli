...
# may carry; "" always means inherit (step ← flow ← global GUI state).
# detail: "auto" follows the GUI's LoD toggle (explicitly — a step can reset a
# flow default), "full" shows everything as authored, "summary" collapses
# containers to tiles. focus: "none" shows the framed viewport as-is,
# "complete" dims elements not completely inside the frame.
DETAIL_VALUES = ("auto", "full", "summary")
FOCUS_VALUES = ("none", "complete")

# Soft cap for bookmark descriptions (playback captions / slide caption cards).
# The caption renders the full text wrapped — the cap is an authoring budget
# enforced in the GUI editors and flagged by export --check, never truncation.
...
class Flow:
    ...
    auto_start: str = ""
    # Flow-wide presentation defaults for the steps; "" = inherit the global
    ...
    for token in rest.split():
        ...
        for seg in parts[1:]:
            ...
            if "=" in seg:
                ...
                if key == "detail" and value in DETAIL_VALUES:
                    step_detail = value
